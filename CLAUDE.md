...
| `providers.py` | Provider table, key resolution, model listing, wiring onto ai-suite's AIService |

## Gotchas
- **Every summary is generated by ai-suite's shared AIService** (the workspace's one AI
  suite). `build.py` packs `ai_suite/service.py` into the zip as `ai_service.py`
  (`plugin_file_source()`), from the sibling `ai-suite` checkout (`AI_SUITE_DIR`) or else the
  vendored `ai_suite/` copy in this repo (synced by ai-suite's `sync.py`; never edit it here),
  and `providers.ai_service_module()` loads it: the zip's copy in Calibre, an unpacked
  build's sibling file, else that same suite source. That module runs on the standard library
...
- **The CLI rows inherit ai-suite's CLI hardening.** Its transports never resolve
