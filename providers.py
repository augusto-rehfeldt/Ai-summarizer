...
# style: how book writer's shared AIService reaches the row (service_overrides()).
...
# ─── completions: book writer's shared AI suite ───────────────────────────
#
# Every summary is generated by book writer's AIService, the AI suite the whole
# workspace shares. build.py ships a copy of its ai_service.py inside the plugin
# zip (Calibre loads plugins from the zip and its Python has no `requests` or
# provider SDKs, which that module tolerates); outside Calibre the checks import
# the original. This module keeps what is the plugin's own: the provider rows,
# key resolution, model listing and the summary cleanup.

BOOK_WRITER = Path(os.environ.get('CALIBRE_SUMMARIZER_BOOK_WRITER')
                   or Path(__file__).resolve().parent.parent / 'book writer')
BOOK_WRITER_AI_SERVICE = BOOK_WRITER / 'ai_book_creator' / 'services' / 'ai_service.py'
...
# Rows whose transport is one of book writer's named providers; every other row is
...
def ai_service_module():
    """book writer's ai_service module: the zip's copy inside Calibre, the original outside."""
    if not _AI_SERVICE:
        try:
            ...
            source = beside if beside.is_file() else BOOK_WRITER_AI_SERVICE
